...
比如测试用例中[[1,2], [2,4], [1,3]]，为什么会删除[1,3]，很明显是[1,3]与[1,2]的区别就是3比2大，如果[1,3] 变成[0,5]呢？<br/>
...
很容易就想到肯定是最大的那支箭，也就是6，因为只有越大，我们才能射爆更多的其他区间。<br/>
...
另外这里看到提示S只包含小写字母'a'到'z'，因为以前做过很多的题目，只要涉及到小写字母'a'到'z'，就联想到了使用一个大小为26的数组来存储a到z出现的次数或者位置<br/>
...
比如输入[[7,0], [4,4], [7,1], [5,0], [6,1], [5,2]]的时候，当发现[7,0]的时候，前面要么没有区间，要么就是身高小于或者等于7且区间的结尾是0的区间。<br/>

那么是不是可以换个角度想，比如[7,0],它应该是放在结果中的0的位置，如果再发现一个比7小，或者等于7的区间，比如[5,0]，按照正常情况下，[5,0]也应该是放在结果的0的位置上的，可以在结果的0的位置上已经有[7,0]了所以[5,0]需要插入到[7,0]的前面。<br/>
...
这里也很简单，就是不断遍历，当发现应该需要序号i的时候，如何根据i前后的数来确定修改为什么。<br/>

这里的贪心可能就是体现在如果根据需要修改的序号i的前后，来确定应该修改为什么。<br/>
